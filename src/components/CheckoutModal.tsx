      ...
      const fetchServerOrders = async () => {
        try {
          ...
          if (resJson.success && resJson.data) {
            ...
            // If we are in tracking mode, let's also update the searchedOrdersList dynamically in real-time
            if (modalMode === 'tracking') {
              const storedLocal = localStorage.getItem("avexon_user_orders");
              if (storedLocal) {
                const parsed = JSON.parse(storedLocal) as Order[];
                if (Array.isArray(parsed) && parsed.length > 0) {
                  setSearchedOrdersList(parsed);
                  if (parsed[0]?.customerPhone) {
                    setSearchOrderId(parsed[0].customerPhone);
                  }
                }
              }
            }
            ...
            if (stored) {
              const parsed = JSON.parse(stored) as Order[];
              setAllOrders(parsed);
              if (modalMode === 'tracking' && parsed.length > 0) {
                setSearchedOrdersList(parsed);
                if (parsed[0]?.customerPhone) {
                  setSearchOrderId(parsed[0].customerPhone);
                }
                  ...
                  <div className="space-y-5 font-sans">
                    <div className="flex items-center justify-between border-b border-purple-500/10 pb-3">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[10px] text-slate-400">
                          ফোন নম্বর: <span className="font-mono text-purple-300 font-bold select-all">{searchOrderId}</span> ({searchedOrdersList.length}টি অর্ডার পাওয়া গেছে)
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 bg-[#0d041d]/75 border border-purple-500/10 rounded-xl">
                        ...
                        <strong className="text-lg font-black text-white">{searchedOrdersList.length} টি</strong>
                      </div>
                      <div className="p-3 bg-[#0d041d]/75 border border-purple-500/10 rounded-xl">
